...
export function dyingCubeSystem(dt: number) {
  ...
  for (const [entity, roomWrapper] of engine.getEntitiesWith(Room)) {
    ...
    if (!roomWrapper.room.loaded) {
      ...
      for (let it = 0; it < 50; it++) {
        const position = Vector3.add(room.topLeftCorner, Vector3.create(room.tempX * room.cubeSize, room.tempLevel * room.levelHeight, room.tempZ * room.cubeSize))
        const cubeIndex = room.tempX  + room.tempZ * roomWrapper.room.cubeAmount + roomWrapper.room.cubeAmount * roomWrapper.room.cubeAmount * room.tempLevel
    ...
    const topLeftCornerCenter = Vector3.add(roomWrapper.room.topLeftCorner, Vector3.create(-roomWrapper.room.cubeSize / 2, 0, -roomWrapper.room.cubeSize / 2))
    ...
    if (playerCubePosition.x < 0 || playerCubePosition.z < 0 || 
    ...
    const cubeIndex = xCube + zCube * roomWrapper.room.cubeAmount + roomWrapper.room.cubeAmount * roomWrapper.room.cubeAmount * level
    const cubeEntity = (roomWrapper.room.cubeStartEntity + cubeIndex + 1) as Entity
