import { AvatarEmoteCommand, PlayerIdentityData, engine, type DeepReadonlyObject, type Entity, type PBAvatarEmoteCommand, AvatarShape } from '@dcl/sdk/ecs'
...
export function main(): void {
  ...
  sceneEntities.pushEntity(utils.addTestCube({ position: Vector3.create(8, 1, 5) }, () => {
    console.log("triggering emote")
    triggerSceneEmote({ src: 'assets/animations/Pose.glb', loop: true }).catch(console.error)
  }))

  sceneEntities.pushEntity(utils.addTestCube({ position: Vector3.create(8, 1, 8) }, () => {
    console.log("triggering emote")
    triggerSceneEmote({ src: 'assets/animations/Crafting_Snowball.glb', loop: false }).catch(console.error)
  }))

  sceneEntities.pushEntity(utils.addTestCube({ position: Vector3.create(8, 1, 12) }, () => {
  ...
  sceneSystems.addSystemWithInverval(
    (_dt) => {
      const playerEmotes = Array.from(AvatarEmoteCommand.get(engine.PlayerEntity))
...
function getEmotes(): Array<[Entity, DeepReadonlyObject<PBAvatarEmoteCommand>]> {
...
export function UI(): JSX.Element {
  ...
  return (
    <UiBox width={400} height={100} uiTransform={{ padding: 10 }}>
      ...
      <Label value={`Users in this scene: ${Array.from(engine.getEntitiesWith(PlayerIdentityData)).length}`} uiTransform={{ height: 30 }} />
      <Label value={`Emotes triggered in this scene: ${emotes.length}`} uiTransform={{ height: 30 }} />
      {emotes.length > 0 && (
        
      <Label value={`${JSON.stringify(emotes[emotes.length-1])}`} uiTransform={{ height: 30 }} />
        )}
    </UiBox>
  )
}
